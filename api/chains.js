...
const CHAINS_PATH = path.join(process.cwd(), 'private', 'chains.json');
...
module.exports = (req, res) => {
  ...
  try {
    const data = fs.readFileSync(CHAINS_PATH, 'utf8');
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    res.setHeader('Cache-Control', 'private, no-store');
    return res.status(200).send(data);
  } catch {
    return res.status(500).json({ error: 'Chain data not found' });
  }
};
