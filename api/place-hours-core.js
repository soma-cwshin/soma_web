...
async function fetchPlaceHoursFromNaver(name, address) {
  const { placeId, searchUrl, searchApollo } = await searchPlaceId(name, address);
  if (!placeId) {
    if (!searchApollo) {
      return { ok: false, error: 'not_found', message: '네이버 검색 결과를 찾지 못했습니다.' };
    }
    return { ok: false, error: 'not_found', message: '업체를 찾지 못했습니다.' };
  }
  ...
  if (!detailApollo) {
    return { ok: false, error: 'not_found', message: '상세 정보를 불러오지 못했습니다.' };
  }
...
module.exports = { fetchPlaceHoursFromNaver };
